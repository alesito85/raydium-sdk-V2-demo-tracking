...
export class JitoJsonRpcClient {
  ...
  async simulateBundle(params: any) {
    const endpoint = this.uuid ? `/bundles?uuid=${this.uuid}` : '/bundles';
    console.debug('Sending bundle:', params);
    return this.sendRequest(endpoint, 'simulateBundle', params);
  }
  ...
  async simulateTxn(params: any, bundleOnly = false) {
    let endpoint = '/transactions';
    const queryParams = [];
    ...
    return this.sendRequest(endpoint, 'simulateTransaction', params);
  }
